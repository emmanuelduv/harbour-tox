...
/*CoreFile::CoreFile(QObject *parent) :
    QObject(parent)
{
}*/
...
unsigned CoreFile::corefileIterationInterval()
{
    ...
    const unsigned fastFileInterval=10, slowFileInterval=50, idleInterval=1000;
...
#else
    QHashIterator<uint64_t, ToxFile> i(fileMap);
    while (i.hasNext()) {
        i.next();
...
void CoreFile::sendAvatarFile(toxWrapper * core, uint32_t friendId, const QByteArray& data)
{
    ...
    uint32_t fileNum = tox_file_send(core->my_tox, friendId, TOX_FILE_KIND_AVATAR, filesize,
                                     avatarHash, avatarHash, TOX_HASH_LENGTH, NULL);
    ...
    tox_file_get_file_id(core->my_tox, friendId, fileNum, (uint8_t*)file.resumeFileId.data(), NULL);
    addFile(friendId, fileNum, file);
}

void CoreFile::sendFile(toxWrapper* core, uint32_t friendId, QString Filename, QString FilePath, long long filesize)
{
    QMutexLocker mlocker(&fileSendMutex);

    QByteArray fileName = Filename.toUtf8();
    uint32_t fileNum = tox_file_send(core->my_tox, friendId, TOX_FILE_KIND_DATA, filesize, NULL,
                                (uint8_t*)fileName.data(), fileName.size(), NULL);
    if (fileNum == std::numeric_limits<uint32_t>::max())
    {
...
void CoreFile::pauseResumeFileSend(toxWrapper* core, uint32_t friendId, uint32_t fileId)
{
    ...
    if (file->status == ToxFile::TRANSMITTING)
    {
        ...
        tox_file_control(core->my_tox, file->friendId, file->fileNum, TOX_FILE_CONTROL_PAUSE, NULL);
    }
    else if (file->status == ToxFile::PAUSED)
    {
        file->status = ToxFile::TRANSMITTING;
        emit core->fileTransferAccepted(*file);
        tox_file_control(core->my_tox, file->friendId, file->fileNum, TOX_FILE_CONTROL_RESUME, NULL);
    }
    else
        qWarning() << "pauseResumeFileSend: File is stopped";
}

void CoreFile::pauseResumeFileRecv(toxWrapper* core, uint32_t friendId, uint32_t fileId)
{
    ToxFile* file = findFile(friendId, fileId);
    if (!file)
    {
...
void CoreFile::cancelFileSend(toxWrapper* core, uint32_t friendId, uint32_t fileId)
{
    ToxFile* file = findFile(friendId, fileId);
    if (!file)
    {
    ...
    tox_file_control(core->my_tox, file->friendId, file->fileNum, TOX_FILE_CONTROL_CANCEL, NULL);
    removeFile(friendId, fileId);
}

void CoreFile::cancelFileRecv(toxWrapper* core, uint32_t friendId, uint32_t fileId)
{
    ToxFile* file = findFile(friendId, fileId);
    if (!file)
    {
...
void CoreFile::rejectFileRecvRequest(toxWrapper* core, uint32_t friendId, uint32_t fileId)
{
    ToxFile* file = findFile(friendId, fileId);
    if (!file)
    {
...
void CoreFile::acceptFileRecvRequest(toxWrapper* core, uint32_t friendId, uint32_t fileId, QString path)
{
    ToxFile* file = findFile(friendId, fileId);
    if (!file)
    {
...
{
    toxWrapper* core = static_cast<toxWrapper*>(_core);
    ...
    if (kind == TOX_FILE_KIND_AVATAR)
    {
        ...
        if (!filesize)
        {
            ...
            QFile::remove(Settings::getInstance(NULL)->getSettingsDirPath()+"avatars/"+friendAddr.left(64)+".png");
            QFile::remove(Settings::getInstance(NULL)->getSettingsDirPath()+"avatars/"+friendAddr.left(64)+".hash");
            return;
        }
        else
        {
            ...
            tox_file_get_file_id(core->my_tox, friendId, fileId, avatarHash, NULL);
            if (Settings::getInstance(NULL)->getAvatarHash(friendAddr) == QByteArray((char*)avatarHash, TOX_HASH_LENGTH))
            {
                ...
                tox_file_control(core->my_tox, friendId, fileId, TOX_FILE_CONTROL_CANCEL, NULL);
                return;
            }
            else
            {
                ...
                tox_file_control(core->my_tox, friendId, fileId, TOX_FILE_CONTROL_RESUME, NULL);
            }
        }
    }
    else
    ...
    file.fileKind = kind;
    file.resumeFileId.resize(TOX_FILE_ID_LENGTH);
    tox_file_get_file_id(core->my_tox, friendId, fileId, (uint8_t*)file.resumeFileId.data(), NULL);
                                 ...
                                 TOX_FILE_CONTROL control, void *core)
{
    ...
    if (control == TOX_FILE_CONTROL_CANCEL)
    {
        ...
        emit static_cast<toxWrapper*>(core)->fileTransferCancelled(*file);
        removeFile(friendId, fileId);
    }
    else if (control == TOX_FILE_CONTROL_PAUSE)
    {
        ...
        file->status = ToxFile::PAUSED;
        emit static_cast<toxWrapper*>(core)->fileTransferRemotePausedUnpaused(*file, true);
    }
    else if (control == TOX_FILE_CONTROL_RESUME)
    {
        qDebug() << "onFileControlCallback: Received pause for file "<<friendId<<":"<<fileId;
        file->status = ToxFile::TRANSMITTING;
        emit static_cast<toxWrapper*>(core)->fileTransferRemotePausedUnpaused(*file, false);
    }
    else
    {
                              ...
                              uint64_t pos, size_t length, void* core)
{
    ...
    if (!length)
    {
        ...
        if (file->fileKind != TOX_FILE_KIND_AVATAR)
        {
            emit static_cast<toxWrapper*>(core)->fileTransferFinished(*file);
            emit static_cast<toxWrapper*>(core)->fileUploadFinished(file->filePath);
        }
        removeFile(friendId, fileId);
        return;
    }
    ...
    if (file->fileKind == TOX_FILE_KIND_AVATAR)
    {
        ...
        nread = chunk.size();
        memcpy(data, chunk.data(), nread);
    }
    else
    {
        file->file->seek(pos);
        nread = file->file->read((char*)data, length);
        if (nread <= 0)
        {
            qWarning("onFileDataCallback: Failed to read from file");
            emit static_cast<toxWrapper*>(core)->fileTransferCancelled(*file);
            tox_file_send_chunk(tox, friendId, fileId, pos, NULL, 0, NULL);
            removeFile(friendId, fileId);
            return;
        }
    ...
    if (!tox_file_send_chunk(tox, friendId, fileId, pos, data, nread, NULL))
    {
        qWarning("onFileDataCallback: Failed to send data chunk");
        return;
    }
    if (file->fileKind != TOX_FILE_KIND_AVATAR)
        emit static_cast<toxWrapper*>(core)->fileTransferInfo(*file);
}
                                    ...
                                    const uint8_t *data, size_t length, void *core)
{
    ...
    if (!file)
    {
        ...
        tox_file_control(tox, friendId, fileId, TOX_FILE_CONTROL_CANCEL, NULL);
        return;
    }
        ...
        if (file->fileKind == TOX_FILE_KIND_AVATAR)
        {
            ...
            if (!pic.isNull())
            {
                qDebug() << "Got "<<file->avatarData.size()<<"bytes of avatar data from"
                          << static_cast<toxWrapper*>(core)->getFriendName(friendId);
                Settings::getInstance(NULL)->saveAvatar(pic, static_cast<toxWrapper*>(core)->getFriendAddress(friendId));
                //Settings::getInstance().saveAvatarHash(file->resumeFileId, static_cast<toxWrapper*>(core)->getFriendAddress(friendId));
                emit static_cast<toxWrapper*>(core)->friendAvatarChanged(friendId, pic);
            }
        }
        else
        {
            emit static_cast<toxWrapper*>(core)->fileTransferFinished(*file);
            emit static_cast<toxWrapper*>(core)->fileDownloadFinished(file->filePath);
        }
        removeFile(friendId, fileId);
        return;
    }
...
void CoreFile::onConnectionStatusChanged(toxWrapper* core, uint32_t friendId, bool online)
{
